...
class DeptTestCase(TestCase):
    ...
    def setUp(self):
        ...
        Dept.valid_objects.create(
            uid='level_2-1', name='level_2-1', parent=level_1, order_no=2
        )
        Dept.valid_objects.create(
            uid='level_2-2', name='level_2-2', parent=level_1, order_no=1
        )
    ...
    def test_get_dept_tree(self):
        res = self.client.get(
            reverse('siteapi:dept_tree', args=('root',)), data={'user_required': True}
        )
        expect = {
            'depts': [
                {
            ...
            'users': [{'name': '', 'user_id': 2, 'username': 'employee'}],
        }
        self.assertEqual(res.json(), expect)

        res = self.client.get(
            reverse('siteapi:dept_tree', args=('root',)), data={'user_required': False}
        )
        expect = {
            'depts': [
                {
    ...
    def test_delete_dept(self):
        ...
        dept_member = DeptMember.objects.create(
    ...
    def test_update_dept(self):
        ...
        res = self.client.json_patch(
            reverse('siteapi:dept_detail', args=('level_1',)), data={'visibility': 1}
        )
        expect = {
            'node_scope': [],
            'user_scope': [],
        ...
        self.assertEqualScoped(
            res.json(), expect, ['node_scope', 'user_scope', 'visibility']
        )

        res = self.client.json_patch(
            reverse('siteapi:dept_detail', args=('level_1',)),
            data={'node_scope': {'as': 'sd'}},
        )
    ...
    def test_create_child_dept(self):
        res = self.client.json_post(
            reverse('siteapi:dept_child_dept', args=('level_1',)),
            data={
                'uid': 'level_2-3',
                'name': 'level_2-3',
                'ding_dept': {
    ...
    def test_update_dept_child_user(self):
        ...
        self.assertEqual(
            set(
                [user['username'] for user in res.json()['results']]
        ...
        self.assertEqual(
            expect, set([user['username'] for user in res.json()['results']])
        )  # pylint:disable=consider-using-set-comprehension

        # test batch move inpalce
        res = self.client.json_patch(
            reverse('siteapi:dept_child_user', args=('another',)),
            data={
                'subject': 'move_out',
                'user_uids': ['employee_3', 'employee_2'],
                'dept_uids': ['another'],
    ...
    def test_create_dept_special_name(self):
        ...
        res = client.json_post(
            reverse('siteapi:node_child_node', args=('d_root',)), data=data
        )
