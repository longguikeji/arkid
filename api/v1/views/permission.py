...
from arkid.core.models import Permission, SystemPermission, OpenPermission
...
def user_add_app_permission(request, tenant_id: str, select_app_id: str, data: PermissionBatchSchemaIn):
    ...
    if data_arr:
        permissiondata = PermissionData()
        permissiondata.add_permission_to_app(tenant_id, select_app_id, data_arr)
        # dispatch_event(Event(tag=ADD_USER_MANY_PERMISSION, tenant=request.tenant, request=request, data={
        #     'user_ids': [select_user_id],
        #     'tenant_id': tenant_id,
        #     'data_arr': data_arr
...
@api.delete("/tenant/{tenant_id}/permission/app/{select_app_id}/{permission_id}/remove_permission", tags=['权限'])
@operation(roles=[TENANT_ADMIN, PLATFORM_ADMIN])
def app_remove_permission(request, tenant_id: str, select_app_id: str, permission_id: str):
    '''
    移除应用权限
    '''
    permission = SystemPermission.valid_objects.filter(id=permission_id).first()
    ...
    if permission:
        from arkid.core.perm.permission_data import PermissionData
        permissiondata = PermissionData()
        permissiondata.remove_permission_to_app(tenant_id, select_app_id, permission)
    return ErrorDict(ErrorCode.OK)
...
def list_user_group_last_permissions(request, tenant_id: str, query_data:UserGroupLastPermQueryIn=Query(...)):
    ...
    from arkid.core.perm.permission_data import PermissionData
    permissiondata = PermissionData()
    return permissiondata.get_user_group_last_permissions(tenant_id, query_data.usergroup_id, query_data.category, query_data.operation_id, query_data.name, query_data.app_name, query_data.app_id)
...
def permission_toggle_other_user_open(request, tenant_id: str, permission_id: str):
    ...
    if permission:
        ...
        if isinstance(permission, SystemPermission):
            # 系统权限
            permissions = SystemPermission.valid_objects.filter(id__in=ids)
            isopen = OpenPermission.valid_objects.filter(
                tenant=tenant,
                system_permission=permission
            ).exists()
            OpenPermission.valid_objects.filter(
                tenant=tenant,
            ...
            if isopen is False:
                for item_permission in permissions:
                    ...
                    open_permission.tenant = tenant
                    open_permission.system_permission = item_permission
                    open_permission.save()
                # 全部打开
            ...
            if isopen is False:
                for item_permission in permissions:
                    open_permission = OpenPermission()
                    open_permission.tenant = tenant
                    open_permission.permission = item_permission
                    open_permission.save()
                # 全部打开
