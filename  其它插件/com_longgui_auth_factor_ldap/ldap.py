...
from ldap3.utils.hashed import hashed, HASHED_SHA
...
class LdapAuth:
    ...
    def authenticate(self, username, password):
        try:
            ...
            if not ldap_user:
                return None, ErrorCode.USER_DOES_NOT_EXIST_IN_LDAP
            dn = ldap_user['dn']
            if not conn.rebind(dn, password):
                return None, ErrorCode.WRONG_PASSWORD
            conn.unbind()
            return ldap_user['attributes'], None
        except Exception as e:
            logger.warning('LDAP connection error: %s' % str(e))
            return None, ErrorCode.LDAP_CONNECT_FAILED
        
    def change_password(self,username,o_password,n_password):
        """更改密码
        ...
        try:
            ...
            ldap_user = self.get_user_from_ldap(conn, username)
            if not ldap_user:
                return False, ErrorCode.USER_DOES_NOT_EXIST_IN_LDAP
            dn = ldap_user['dn']
            if not conn.rebind(dn, o_password):
            ...
            logger.warning('LDAP connection error: %s' % str(e))
            return False, ErrorCode.LDAP_CONNECT_FAILED
