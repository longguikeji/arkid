...
class PermissionData(object):
    ...
    def update_app_all_permission(self, tenant, app):
        ...
        if permissions_data:
            ...
            for group_item in group_data:
                ...
                group_sort_ids = []
                # 判断是否是新分组(用于解决分组更新的问题>旧)
                if group_permission.container.exists():
                    change_group_info = {}

                    change_group_info_list = []
                    for item_system_obj in group_permission.container.all():
                        change_group_info_list.append(item_system_obj.sort_id)
                    if change_group_info_list:
                        change_group_info_list.sort()
                    change_group_info['source'] = change_group_info_list

                    change_group_infos[str(group_permission.id)] = change_group_info
                # 需要先清理在添加
                ...
                for api_item in api_data:
                    ...
                    if sort_id in container and api_permission:
                        ...
                        group_sort_ids.append(sort_real_id)
                # 判断是否是新分组(用于解决分组更新的问题>新)
                if str(group_permission.id) in change_group_infos:
                    change_group_info = change_group_infos.get(str(group_permission.id))
    ...
    def update_app_all_user_permission(self, tenant, app, change_sort_ids=None):
        '''
        更新应用所有用户权限
        '''
        # 取得当前租户的所有用户
        ...
        if app_id != 'arkid':
            ...
            for apppermissionresult in apppermissionresults:
                ...
                if apppermissionresult.app:
                    result_items = Permission.valid_objects.filter(
                        tenant_id=tenant_id,
                        app_id=app_id,
            ...
            if user_id:
                ...
                for userpermissionresult in userpermissionresults:
                    ...
                    if permission_sort_ids:
                        for item in permissions.filter(app=userpermissionresult.app, sort_id__in=permission_sort_ids):
    ...
    def get_user_app_last_permissions(self, tenant_id, app_id, user_id, category=None, operation_id=None, name=None, app_name=None):
        ...
        if userpermissionresult:
            ...
            index_list = []
            for index,value in enumerate(permission_result_list):
                if int(value) == 1:
