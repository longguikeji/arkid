...
from scim_server.views.users_view import UsersView
from scim_server.views.groups_view import GroupsView
...
urlpatterns = [
    ...
    re_path(r'^Users(?:/(?P<uuid>[^/]+))?$', UsersView.as_view(), name='users'),
    # re_path(r'^Groups/.search$', views.GroupSearchView.as_view(), name='groups-search'),
    re_path(r'^Groups(?:/(?P<uuid>[^/]+))?$', GroupsView.as_view(), name='groups'),
