...
class WeChatScanExternalIdpProvider(ExternalIdpProvider):
    ...
    def create(self, tenant_uuid, external_idp, data):
        ...
        appid = data.get('appid')
        secret = data.get('secret')
        login_url = host+reverse("api:wechatscan:login", args=[tenant_uuid])
        bind_url = host+reverse("api:wechatscan:bind", args=[tenant_uuid])
        userinfo_url = host+reverse("api:wechatscan:userinfo", args=[tenant_uuid])
