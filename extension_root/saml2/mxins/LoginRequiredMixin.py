...
class LoginRequiredMixin(AccessMixin):
    ...
    def dispatch(self, request, tenant_uuid, app_id, *args, **kwargs):
        ...
        '''
        try:
            spauthn = request.GET.get("spauthn", None) or request.COOKIES.get("spauthn",None) or request.GET.get("token",None)
            ...
            logger.debug(err)
            next = f"{request.path}?{'&'.join([f'{k}={request.GET[k]}'for k in request.GET.keys() if k!='spauthn'])}"
