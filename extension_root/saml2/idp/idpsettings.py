...
def get_saml_idp_config(tenant_uuid, app_id):
    ...
    创建SAML_IDP_CONFIG
    """
    sp_metadata_path = os.path.join(MD_BASEDIR, f'{tenant_uuid}_{app_id}_sp.xml')
    ...
    saml_idp_config = {
        ...
        'entityid': f'{get_app_config().get_frontend_host()}{reverse("api:saml2:idp_metadata",args=(tenant_uuid,app_id))}',
        'description':
            ...
            'idp': {
                ...
                'endpoints': {
                    'single_sign_on_service': [
                        (f'{get_app_config().get_host()}{reverse("api:saml2:idp_sso_post",args=(tenant_uuid,app_id))}', BINDING_HTTP_POST),
                        (f'{get_app_config().get_frontend_host()}{reverse("api:saml2:idp_sso_post",args=(tenant_uuid,app_id))}', BINDING_HTTP_POST),
                        (f'{get_app_config().get_host()}{reverse("api:saml2:idp_sso_redirect",args=(tenant_uuid,app_id))}', BINDING_HTTP_REDIRECT),
                        (f'{get_app_config().get_frontend_host()}{reverse("api:saml2:idp_sso_redirect",args=(tenant_uuid,app_id))}', BINDING_HTTP_REDIRECT),
                    ],
                },
