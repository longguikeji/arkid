...
def get_sp_config(app: App):
    ...
    """
    sp_config = {
        'processor': 'extension_root.saml2.idp.processors.BaseProcessor',
        'attribute_mapping': {
            'email': 'email',
        ...
        'extra_config': app.data.get("attribute_mapping", {})
    }
    return sp_config
...
class LoginProcess(LoginRequiredMixin, IdPHandlerViewMixin, View):
    ...
    def get(self, request, tenant__uuid, app_id):  # pylint: disable=unused-argument
        ...
        sp_config = get_sp_config(app)
        # Check if user has access
        try:
            # Check if user has access to SP
            check_access(self.get_processor(
                resp_args['sp_entity_id'], sp_config), request)
    ...
    def build_authn_response(self, user, authn, resp_args, sp_config, app) -> list:  # type: ignore
        ...
        processor: BaseProcessor = self.get_processor(resp_args['sp_entity_id'], sp_config)  # type: ignore
        user_id = processor.get_user_id(user)
        name_id = NameID(format=name_id_format,
                        sp_name_qualifier=resp_args["sp_entity_id"], text=user_id)
        if app.data.get("xml_data", None):
            _spsso_descriptor = entity_descriptor_from_string(
                app.data["xmldata"]).spsso_descriptor.pop()
            ...
            sign_response = app.data.get("sign_response", True)
            sign_assertion = app.data.get("sign_assertion", True)
            
        encrypt_saml_responses = app.data.get("encrypt_saml_responses", False)
