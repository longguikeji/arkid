...
class MobileAuthFactorExtension(AuthFactorExtension):
    ...
    def authenticate(self, event, **kwargs):
        ...
        if user:
            ...
            if user_mobile:
                if check_mobile(mobile, user_mobile.mobile):
                    return self.auth_success(user)
        
        return self.auth_failed(event, data={'error': ErrorCode.USERNAME_MOBILE_MISMATCH.value, 'message': 'username or mobile not correct'})
    ...
    def register(self, event, **kwargs):
        ...
        ret, message = self.check_mobile_complexity(mobile, config)
        if not ret:
            data = {
                'error': ErrorCode.MOBILE_STRENGTH_ERROR.value,
                'message': message,
            }
            return data
        ...
        if not register_fields:
            ...
            if request.POST.get('username') is None:
                self.auth_failed(event, data={'error': ErrorCode.USERNAME_MOBILE_MISMATCH.value, 'message': 'username不能为空'})
        else:
            fields = [k for k in register_fields if request.POST.get(k) is not None]
            if not fields:
                self.auth_failed(event, data={'error': ErrorCode.USERNAME_MOBILE_MISMATCH.value, 'message': '所有用户标识至少填一个'})
        ...
        user.mobile = make_mobile(mobile)
        user.save()
    ...
    def check_mobile_complexity(self, pwd, config):
        if not pwd:
            return False, 'No mobile provide'
