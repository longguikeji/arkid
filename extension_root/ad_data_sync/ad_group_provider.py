...
class AdGroupProvider(ProviderBase):
    ...
    def __init__(self, config, group_attr_map=None):
        ...
        self.group_attr_map = {'objectGUID': 'identifier', 'name': 'display_name'}
    ...
    def convert_ad_ou_to_scim_group(self, ad_user):
        ad_group_attrs = ad_user['attributes']
        group = Core2Group()
        for attr1, attr2 in self.group_attr_map.items():
            ad_value = ad_group_attrs.get(attr1)
            setattr(group, attr2, ad_value)
        return group
