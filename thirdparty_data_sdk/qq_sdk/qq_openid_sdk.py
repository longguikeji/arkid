...
class QqInfoManager():
    ...
    def get_open_id(self, code, redirect_uri):
        '''
        获取qq的openid
        '''
        try:
    ...
    def check_config_valid(self, redirect_uri):    # pylint: disable=no-self-use
        '''
        检查配置是否正确
        '''
        try:
            accesss_token_info = requests.get(constants.GET_TOKEN_URL, params={'grant_type':'authorization_code',\
                'client_id':self.app_id, 'client_secret':self.app_key, 'redirect_uri': redirect_uri}).text
            errcode = re.findall(r'"error":(.*?),', accesss_token_info)[0]
            if errcode == constants.APPID_KEY_VALID:
                return True
            if errcode == constants.REDIRECT_URI_ILLEGAL:
                raise ValidationError({'redirect_uri': ['illegal']})
            return False
