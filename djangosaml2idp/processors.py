from djangosaml2idp.models.ServiceProviderModel import ServiceProvider
import hashlib
import logging
...
class NameIdBuilder:
    ...
    def get_nameid_email(cls, user_id: str, user: settings.AUTH_USER_MODEL = None, **kwargs) -> str:
        if '@' not in user_id:
            raise Exception(
                f"user_id {user_id} does not contain the '@' symbol, so is not a valid NameID Email address format.")
        return user_id
    ...
    def get_nameid(cls, user_id: str, nameid_format: str, **kwargs) -> str:
        ...
        if not method:
            raise NotImplementedError(
                f'{nameid_format} has not been mapped in NameIdBuilder.format_mappings')
        if not hasattr(cls, method):
            raise NotImplementedError(
                f'{nameid_format} has not been implemented NameIdBuilder methods')
