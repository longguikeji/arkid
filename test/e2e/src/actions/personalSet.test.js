...
test('一账通-修改密码', async () => {
    ...
    const usernameInput = await page.waitForSelector('input[type = "text"]');
    await usernameInput.type("bumen2user");
    const passwordInput = await page.waitForSelector('input[type = "password"]');
    await passwordInput.type("bumen2user");
    ...
    await usernameInput1.type("bumen2user");
    const passwordInput1 = await page.waitForSelector('input[type = "password"]');
    await passwordInput1.type("bumen2111");
